...
- 🖼️ **PNG スペクトル画像クイックルック & 前後ナビゲーション (Sequential Browsing)**
  - サムネイルまたは天球マップから瞬時に高解像度 PNG スペクトル画像モーダルを起動。
  - ヘッダー・フッターのボタンおよび画像の左右フローティング矢印（`❮` / `❯`）により、モーダルを閉じることなく前後の天体を連続閲覧可能。
  - キーボードショートカット（`←` / `→`）による快適なスライドショー閲覧に対応。
  - ページ末尾に到達した際は自動で次ページを読み込み、シームレスに天体を継続表示。
  - 天体移動に合わせて背景テーブルの該当行が連動ハイライト。
